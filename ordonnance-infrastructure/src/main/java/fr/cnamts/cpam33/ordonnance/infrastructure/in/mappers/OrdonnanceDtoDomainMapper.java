...
public interface OrdonnanceDtoDomainMapper {
    ...
    default MedecinId mapToMedecinId(MedecinIdDto dto) {
        boolean isNotMappable = dto == null
                || dto.externalId() == null
                || dto.rpps() == null
                || dto.externalId().isBlank()
                || dto.rpps().isBlank();
        return isNotMappable
                ? null
                : new MedecinId(dto.externalId(), new Rpps(dto.rpps()));
    }

    default PatientIdDto mapToPatientIdDto(PatientId patientId) {
        return patientId != null
                ? new PatientIdDto(patientId.numero())
                : null;
    }

    default MedecinIdDto mapToMedecinIdDto(MedecinId medecinId) {
        return medecinId != null
                ? new MedecinIdDto(medecinId.id(), medecinId.rpps().numero())
                : null;
    }
